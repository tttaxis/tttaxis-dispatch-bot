...
import MessageBird from "@messagebird/sdk";
...
if (process.env.MESSAGEBIRD_API_KEY) {
  try {
    mbClient = MessageBird(process.env.MESSAGEBIRD_API_KEY);
  } catch (err) {
    console.error("MESSAGEBIRD INIT ERROR:", err.message);
  }
} else {
  console.warn("MessageBird not configured — SMS disabled");
}
...
async function sendSMSBooking(booking) {
  ...
  try {
    await mbClient.messages.create({
      originator: process.env.MESSAGEBIRD_FROM,   // e.g. "TTTaxis"
      recipients: [process.env.DISPATCH_SMS_TO],  // +44...
      body: message
    });
  } catch (err) {
    console.error("MESSAGEBIRD SMS ERROR:", err.message);
  }
}
